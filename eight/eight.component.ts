...
export class EightComponent {
  ...
  messages = ['Elérkeztünk az egyik utolsó feladatodhoz! Összeteszünk most egy komolyabb játékot MicroBitre!', "A feladat az lenne, hogy írjunk egy számkitaláló játékot! Generálj egy random számot, helyezd el egy változóban!", "A bal gomb nyomkodásával növelni lehet a képernyőn lévő számot 1-9-ig! A jobb oldali gombbal tippelünk az éppen adott számmal", "Ha nagyobb akkor jelenjen megy egy felfele mutató nyil. Ha kisebb akkor egy lefele!", "Ha eltaláltuk akkor jelenjen meg egy szivecske! A változókat nevezd el 'valtozo'-nak és 'proba'-nak, hogy tudjuk majd ellenőrizni a munkád!"]
  /**
   *
   */
  constructor(private serialService : SerialService ,private numberService : NumberService, private microBitService : MicroBitService, private router : Router, private fieldService : FieldService) {
...
valtozo = (Math.floor(Math.random() * (9 - 1 + 1)) + 1);
proba = 0;
  ...
  proba = (typeof proba === 'number' ? proba : 0) + 1;
    ...
    setWatch(function() {
        if (proba == valtozo) {
    show(4685802);} else if (proba < valtozo) {
    show(4347332);} else {
    show(4674692);}
      ...
      if (code.replace(/\s+/g, "") == expected.replace(/\s+/g, ""))
        {
          ...
          setTimeout(() => {
            ...
            this.fieldService.completeLevel("8")
          }, 3000);
        }
